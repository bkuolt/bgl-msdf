...
template<typename T>
cl::Buffer make_buffer(cl::Context& context, int size ) {
  return cl::Buffer(context, CL_MEM_READ_WRITE  | CL_MEM_COPY_HOST_PTR, sizeof(T) * size, nullptr);
}
...
int RunOpenCL() {
    ...
    try { 
        ...
        // 5) Buffers + Upload
        spdlog::info("creating buffers and uploading ({} MB)", (sizeof(float) * N * 3) / (1024 * 1024));
